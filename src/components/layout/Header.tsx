  ...
  useEffect(() => {
    ...
    if (user) {
      try {
        ...
        if (parsed?.name && parsed?.email) setCurrentUser({ name: parsed.name, email: parsed.email });
      } catch {
        setCurrentUser(null);
      }
    }
  ...
  return (
    <header className="flex items-center justify-between px-4 sm:px-6 py-4 sm:py-6 bg-[#2596BE] relative">
      <motion.img
        ...
        onClick={() => (window.location.href = "/")} // redirige al inicio
      />
      ...
      <nav className={`${menuOpen ? "flex flex-col absolute top-full left-0 w-full bg-[#2596BE] shadow-lg z-10" : "hidden"} sm:flex sm:static sm:flex-row sm:gap-6`}>
        ...
        {currentUser ? (
          <div className="flex flex-col sm:flex-row gap-3 py-3 sm:py-0 sm:ml-6 border-t sm:border-none border-white/20">
          ...
          <div className="flex flex-col sm:flex-row gap-3 py-4 sm:py-0 sm:ml-4">
            <Link to="/login">
              <Button className="bg-white text-[#2596BE] hover:bg-[#00c0e8] hover:text-black w-full sm:w-auto">Inicia sesión</Button>
            </Link>
            <Link to="/register">
              <Button className="bg-[#00c0e8] hover:bg-[#00a8d0] text-white w-full sm:w-auto">Regístrate</Button>
            </Link>
          </div>
        )}
      </nav>
